...
export function GithubRepo({ p }: { p: GithubRepoPage }) {
  return (
    <div
      className="flex h-full min-h-0 w-full flex-col overflow-hidden bg-white font-body text-[#1f2328]"
      style={p.zoom ? { zoom: p.zoom } : undefined}
    >
      {/* nav */}
      ...
      <div className="flex shrink-0 items-center gap-2 border-b border-neutral-200 px-8 pt-5 pb-3">
        ...
        {p.nameBadge ? <NumberBadge n={p.nameBadge} size="sm" /> : null}
      ...
      <div className="flex min-h-0 flex-1 gap-6 overflow-hidden px-8 py-4">
        <div className="flex min-w-0 flex-[2.6] flex-col gap-3 overflow-hidden">
          ...
          <div className="flex items-center gap-2 rounded-t-md border border-b-0 border-neutral-200 bg-neutral-50 px-4 py-2 text-[14px]">
            {p.lastCommit.badge ? <NumberBadge n={p.lastCommit.badge} size="sm" /> : null}
          ...
          <div className="flex flex-col overflow-hidden rounded-b-md border border-neutral-200 text-[14px]">
            {p.files.map((f, i) => (
              <div key={i} className="flex items-center gap-2 border-t border-neutral-100 px-4 py-2 first:border-t-0">
          ...
          {p.readme ? (
            <div className="flex flex-1 flex-col overflow-hidden rounded-md border border-neutral-200">
              <div className="flex items-center gap-2 border-b border-neutral-200 bg-neutral-50 px-4 py-2.5 text-[14px] text-neutral-700">
                {p.readme.badge ? <NumberBadge n={p.readme.badge} size="sm" /> : null}
            ...
            <div className="flex flex-col items-center justify-center gap-3 rounded-md border border-neutral-200 p-8 text-center">
              <div className="flex items-center gap-2">
                {p.readmeBadge ? <NumberBadge n={p.readmeBadge} size="sm" /> : null}
