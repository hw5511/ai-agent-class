# -*- coding: utf-8 -*-
"""BASIC 01 · STEP 5 — agy(Antigravity) 실습 슬라이드 7장 재생성.

슬라이드 30~36. CEO 지시: agy_실행_화면.svg(29번)을 시각 기준으로 삼아,
VS Code 창의 터미널 패널 자리에 실제 Antigravity 세션 화면(agent_panel)을
올려서, 학생이 에이전트 세션·타이핑한 프롬프트·탐색기/편집기 결과를
한 화면에서 같이 보게 한다.

실습 흐름: agy 에게 "너는 누구니?" 질문 -> gemini.txt 작성 요청 ->
파일 생성 확인 -> introduce 폴더 생성 -> 파일을 폴더로 이동 -> 최종 폴더
구조 확인 -> Ctrl+C 로 세션 종료.

폴더/파일 이름은 고정: 실습 폴더 = 에이전트1, 파일 = gemini.txt,
...
INTRO_TXT_SHORT = [
    (1, '# 자기소개서', None),
    (2, '안녕하세요! 저는 Antigravity입니다.', None),
...
s.add(section(190, 'VS CODE'))
s.add(vscode(
    tree=[(0, CWD, 'folder', False), (1, 'gemini.txt', 'file', True)],
    tab='gemini.txt', editor=INTRO_TXT, term=[]))
s.add(agent_panel(740, 238, 480, 384, agent='antigravity', version='0.6',
                   cwd=CWD,
                   lines=[
                       ('user', "현재 폴더에 'gemini.txt'를 만들어줘.", None),
                       ('user', '네가 누구이고 무엇을 할 수 있는지', None),
                       ('tool', '간단히 작성해줘.', None),
                       ('agent', "✔ gemini.txt 파일을 만들었습니다.", None),
                       ('tool', '탐색기에 파일이 추가되고 편집기에서', None),
                       ('tool', '내용이 열렸습니다.', None),
                   ],
                   placeholder='좌측 편집기와 탐색기에서 생성 결과를 확인하세요.'))
save('agy_자기소개서_생성.svg', s)
...
s.add(agent_panel(740, 238, 480, 384, agent='antigravity', version='0.6',
                   cwd=CWD,
                   lines=[
                       ('agent', '✔ 완료했습니다. gemini.txt를 생성했습니다.', None),
...
s.add(vscode(
    tree=[(0, CWD, 'folder', False),
          ...
          (1, 'gemini.txt', 'file', False)],
    tab='gemini.txt', editor=INTRO_TXT_SHORT, term=[]))
s.add(agent_panel(740, 238, 480, 384, agent='antigravity', version='0.6',
                   cwd=CWD,
                   lines=[
                   ...
                   placeholder='무엇을 도와드릴까요?'))
save('agy_introduce_폴더_생성.svg', s)
...
s.add(vscode(
    tree=[(0, CWD, 'folder', False),
          ...
          (2, 'gemini.txt', 'file', True)],
    tab='introduce/gemini.txt', editor=INTRO_TXT_MOVED, term=[]))
s.add(agent_panel(740, 238, 480, 384, agent='antigravity', version='0.6',
                   cwd=CWD,
                   lines=[
                       ('user', "'gemini.txt'를 'introduce' 폴더로", None),
                       ('tool', '옮겨줘.', None),
                       ('agent', '✔ 파일을 introduce 폴더로 이동했습니다.', None),
                       ('tool', 'introduce\\gemini.txt 로 이동 완료.', None),
                   ],
                   placeholder='무엇을 도와드릴까요?'))
save('agy_자기소개서_이동.svg', s)
...
s.add(agent_panel(740, 238, 480, 384, agent='antigravity', version='0.6',
                   ...
                   lines=[
                       ...
                       ('tool', '요청한 대로입니다.', None),
                       ('tool', 'introduce\\gemini.txt', None),
                       ('tool', '다음 단계: Ctrl + C로', None),
                       ('tool', 'Antigravity CLI 종료', None),
                   ],
                   placeholder='무엇을 도와드릴까요?'))
save('agy_최종_폴더_구조.svg', s)


# 7. Ctrl + C 로 CLI 종료 -----------------------------------------------
s = Slide(BADGE, STEP, 'Ctrl + C로 CLI 종료',
          '실습을 마친 뒤 Antigravity 세션을 정상 종료합니다',
...
s.add(vscode(
    tree=[(0, CWD, 'folder', False),
          (1, 'introduce', 'folder', False),
          (2, 'gemini.txt', 'file', True)],
    tab='introduce/gemini.txt', editor=INTRO_TXT_SHORT, term=[]))
s.add(agent_panel(740, 238, 480, 384, agent='antigravity', version='0.6',
                   cwd=CWD,
                   lines=[
                       ('user', '무엇을 도와드릴까요? [입력 대기]', None),
                       ('tool', '^C', '#f87171'),
                       ('tool', 'Antigravity CLI 세션을 정상 종료하고', None),
                       ('tool', '일반 터미널로 복귀합니다.', None),
                   ],
                   placeholder=None, caret=False))
save('agy_Ctrl_C_종료.svg', s)

print('done: 7 slides regenerated')
